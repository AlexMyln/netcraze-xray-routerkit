...
Network acquisition accepts only HTTPS on port 443, with no URL userinfo or fragments. Outer whitespace around one complete HTTPS source is removed consistently for hidden, environment, protected-file, and generator input, so LF/CRLF file endings work; internal whitespace, control characters, multiple lines, and empty values are rejected, while raw/offline payloads are unchanged. Every redirect is independently URL-validated and DNS-resolved; every returned address must pass immutable version-independent special-purpose CIDR policy, the TCP connection is pinned to a validated address, TLS still verifies the original hostname, and the connected peer is checked. IPv4-mapped IPv6, standardized NAT64, Teredo, 6to4, and ORCHID ranges are conservatively rejected. Cancellation stops retries/redirects while resources are closed or reaped; CLI interruption is generic and writes no file. The limits are 5 redirects, 16 DNS addresses per hop, 5 seconds per DNS hop, 10 seconds per address connection, a 30-second operational deadline plus bounded cleanup grace, an 8192-byte URL/redirect value, and a 1 MiB response. Python 3.8 and the primary CI `3.x` runtime exercise the address policy. Compressed responses, JavaScript redirects, and HTML meta refresh are not supported. `profile-source --dry-run` may perform this network read and parsing but never writes `profiles.json`. The generator's existing `subscription_url` and `subscription_url_env` fields use the same resolver. See the [network security ADR](docs/architecture/profile-source-network-security.md).
