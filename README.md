...
The generator emits a durable owner-only `routerkit.local-endpoints.v1` manifest containing only slots, fixed code-owned labels (`primary`, `fallback-1`, `fallback-2`), loopback addresses, ports 1082–1084, enabled state, and SOCKS5 protocol. Raw profile names never enter it. POSIX publication directories must be exact owner-private `0700`; unsafe existing directories/files are rejected without chmod or replacement, the parent directory is synchronized, and a validated stale manifest is retired before current generation. The planner validates all snapshot references and every directly supplied selected-device reference, uses the shared #21/#15 trusted-unicast MAC normalizer, embeds the complete desired profiles, and binds each plan to the exact canonical source-snapshot semantics. Simulation accepts no independent manifest, rejects snapshot mismatch or plan tampering before mutation, verifies actual synthetic objects, and proves reuse-only idempotent reruns. Local integrity/snapshot fingerprints never enter public evidence; the separate public fingerprint covers the complete canonical default projection through a minimized digest and is evidence only, not write authorization. Existing updates and assignment moves remain blocked in this fixture-first path. `setup --plan-netcraze --netcraze-state-file PATH` runs the offline stage after optional #21 selection; when combined with `--apply`, confirmation and `--yes` output explicitly state that Netcraze actions are excluded. The integrated setup path still has no native Netcraze apply stage. Hardware-confirmed standalone execution uses the preserved local `ndmc` adapter or the protected external transaction protocol for official MCP/RMM without arbitrary Entware shell; see the [transport architecture](docs/architecture/netcraze-external-transport.md) and [live runbook](docs/live-install-runbook.md). The fixture-first verdict remains `SOFTWARE_PLAN_CORE_READY_HARDWARE_WRITE_CONTRACT_PENDING`; see the [planning architecture](docs/architecture/netcraze-policy-plan.md), [interface research](docs/research/netcraze-policy-interface.md), and [hardware packet](docs/hardware/netcraze-policy-contract.md).

The consolidated offline hardware-canary package now links #21, #15, and the complete #16 matrix without enabling hardware access. `python3 scripts/routerkit-hardware-canary.py status|validate|render|matrix` reads only the committed secret-free packet and repository references; the consolidated shell probe has print-only modes and no execution path. A valid repository gate reports `READY_FOR_HARDWARE_CANARY` together with `hardware_validated=false` and `live_contract_confirmed=false`. The 120-minute operator flow protects a 15-minute cleanup/device-return reserve and covers read-contract capture, explicitly authorized disposable writes, the full alpha.16 software path, rerun, safe failures, reboot/recovery, evidence, rollback, and cleanup. It remains development/operator tooling and is not part of normal `routerkit setup`. See the [primary runbook](docs/hardware/netcraze-hardware-canary.md), [printable checklist](docs/hardware/netcraze-canary-checklist.md), and [readiness architecture](docs/architecture/hardware-canary-readiness.md).
...
Default profile-source setup integration completed [#24](https://github.com/AlexMyln/netcraze-xray-routerkit/issues/24) and parent [#20](https://github.com/AlexMyln/netcraze-xray-routerkit/issues/20). The standalone bootstrap transaction now also has an explicit setup gate; live device discovery hardware confirmation, policies, and hardware validation remain separate work.
