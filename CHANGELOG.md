...
- `S23xray-direct` now detects stale lock owners but does not automatically delete stale or unclear lock paths; operators must inspect the lock manually, avoiding POSIX shell path deletion that cannot be bound atomically to a previously proven inode identity.
...
- Hardened hardware-canary readiness so the committed packet must match canonical phase/check/cleanup/rollback contracts, every hard-timeout route fits the 120-minute window, custom packets cannot self-certify READY, and evidence manifests enforce released-baseline provenance plus timestamp, check-outcome, cleanup, and final-outcome lifecycle rules.
