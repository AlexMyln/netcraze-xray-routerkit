...
Generator создаёт owner-only `generated/routerkit-local-endpoints.json` с fixed code-owned labels, validated no-clobber replacement, file/parent-directory `fsync` и удалением stale manifest до current generation. Raw profile names и upstream secrets туда не попадают. Setup читает его после generation и strict plan; optional selection выполняется раньше, validated Netcraze plan — до apply confirmation. Conflict, orphan reference, inconsistent default, stale state, update existing object или assignment move останавливают setup. При сочетании с `--apply` непосредственно перед confirmation или первым `--yes` mutation печатается блок: Netcraze actions — только offline preview и исключены из RouterKit apply. `simulate` revalidates каждую mutation и rollback, но остаётся synthetic software evidence. Netcraze apply-команды нет; статус остаётся `SOFTWARE_PLAN_CORE_READY_HARDWARE_WRITE_CONTRACT_PENDING`.
