...
Explicit device discovery является read-only и fixture-first:
...
Generator создаёт owner-only `generated/routerkit-local-endpoints.json` с fixed code-owned labels, validated no-clobber replacement, file/parent-directory `fsync` и удалением stale manifest до current generation. На POSIX generated directory должна иметь exact `0700`; reused public/traversable directory отвергается без chmod и residue. Raw profile names и upstream secrets туда не попадают. Setup читает manifest после generation и strict plan; optional selection выполняется раньше и повторно проходит общий assignment-safe MAC boundary. `ChangePlan` встраивает exact desired endpoint semantics и привязывается к canonical source snapshot. `simulate` принимает только plan и тот же snapshot, до mutation отвергает mismatch/tampering, проверяет actual synthetic state и доказывает idempotent reuse-only rerun. Local integrity/snapshot fingerprints остаются local; public evidence использует отдельный minimized fingerprint с полным default-semantics coverage и никогда не разрешает write. Conflict, orphan reference, inconsistent default, stale state, invalid selected device, update existing object или assignment move останавливают setup. При сочетании с `--apply` непосредственно перед confirmation или первым `--yes` mutation печатается блок: Netcraze actions — только offline preview и исключены из RouterKit apply. Netcraze apply-команды нет; статус остаётся `SOFTWARE_PLAN_CORE_READY_HARDWARE_WRITE_CONTRACT_PENDING`.

### Consolidated hardware-canary preparation

Ограниченное hardware window отделено от normal setup. В repository есть versioned secret-free packet и pure offline validator:
...
`READY_FOR_HARDWARE_CANARY` означает, что подготовлены operator packet, read/write contract capture, full #16 matrix, evidence schema, time budget, rollback, cleanup и device return. Результат всегда содержит `hardware_validated=false` и `live_contract_confirmed=false`. Packet не соединяется с router, не добавляет live adapter/write mode и не вызывается через `routerkit setup`. См. [основной runbook](hardware/netcraze-hardware-canary.ru.md) и [readiness contract](architecture/hardware-canary-readiness.ru.md).
