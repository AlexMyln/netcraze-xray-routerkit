...
The live Netcraze/Keenetic adapter reports `contract_unverified` until the hardware probe confirms whether the target-firmware contract should use CLI, `/rci`, another structured interface, or a combination. The inventory-file mode is for synthetic offline validation only and requires an owner-only regular file with no symlink or hardlink. Selection always includes option `0` for no assignment; blank input and EOF also choose no assignment. Any nonzero choice requires complete trusted evidence: adapter state `supported`, no sanitized errors, all sources `supported`, and a valid unicast MAC identity. Fixture data cannot turn vendor IDs, router IDs, unknown identifiers, or IP-only records into assignment identities. The stage may pass the `DeviceSelection` directly into the optional #15 offline planner, but it never serializes, prints, or persists the ephemeral handle. It executes no router command, writes no policy, creates no proxy connection, changes no default policy, performs no active scan, and does not persist real inventories.

The explicit #15 stage is plan-only:
...
Generation writes `generated/routerkit-local-endpoints.json` with owner-only permissions, fixed code-owned labels, validated no-clobber replacement, file and parent-directory synchronization, and stale-manifest retirement before current generation. It contains no raw profile names or upstream profile secrets. Setup reads it only after generation and strict install planning; optional device selection runs first, then the validated Netcraze plan runs before any apply confirmation. A conflict, orphan reference, inconsistent default, stale state, existing-object update, or assignment move stops setup. When combined with `--apply`, an exclusion block immediately before confirmation or the first `--yes` mutation states that Netcraze actions are offline preview only and excluded from RouterKit apply. `simulate` revalidates every mutation and rollback but remains synthetic software evidence, not target hardware behavior. No Netcraze apply command exists, and the status remains `SOFTWARE_PLAN_CORE_READY_HARDWARE_WRITE_CONTRACT_PENDING`.
...
This integration functionally completes #29 and parent #13 but is not the end of epic #5. Autostart is #14; #15 now has a fixture-first software plan core but still needs the hardware write contract; read-only fixture-first device discovery is #21 software work with hardware contract confirmation still pending; hardware validation is #16. The path is not hardware-tested until #16 is complete. Plain `setup` and `setup --apply` do not bootstrap, enable autostart, discover devices, or plan Netcraze state; only explicit flags add those stages. No setup mode activates Entware, proves reboot persistence, writes Netcraze policies or the default policy, automates the Web UI, creates firewall/TPROXY/REDIRECT rules, or calls `xkeen -start`. Generated fragments remain secret-bearing local operational artifacts and must not be published.
