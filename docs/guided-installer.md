...
Generation writes `generated/routerkit-local-endpoints.json` atomically with owner-only permissions. It contains only safe local endpoint metadata, not upstream profile secrets. Setup reads it only after generation and strict install planning; optional device selection runs first, then the Netcraze plan runs before any apply confirmation. A conflict stops setup. Success reports that no connection, policy, assignment, or default-policy write occurred. `simulate` is available only with `--fixture-simulation`; it proves software rollback logic, not target hardware behavior. No Netcraze apply command exists, and the status remains `SOFTWARE_PLAN_CORE_READY_HARDWARE_WRITE_CONTRACT_PENDING`.
