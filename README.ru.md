...
Совместимость платформ и прошивок: [docs/hardware/platform-compatibility.ru.md](docs/hardware/platform-compatibility.ru.md)
...
Runtime execution и native configuration — независимые оси. `local-router`
разрешает существующие subprocess stages preflight/bootstrap/setup/backup/
install/healthcheck/autostart и должен быть выбран явно для mutable запуска на
самом target router. `external-evidence` никогда не выполняет эти stages и не
пишет в `/opt` orchestrator host. Он либо принимает existing runtime через
`--adopt-existing-runtime` со strict fresh runtime evidence и явно переданным
RouterKit-valid endpoint manifest, либо возвращает
`ROUTERKIT_RUNTIME_EXECUTION_REQUIRED` для shell-capable handoff на target.
Adoption не читает profile source и не регенерирует config; receipt различает
runtime outcomes `ADOPTED`, local `EXECUTED` и `EXTERNAL_EXECUTED`.

`local-ndmc` делегирует native Proxy/Policy stage существующему reviewed local
adapter. `external` управляет только native router configuration и
строит/проверяет `routerkit.netcraze.external-transaction.v1`; official MCP/RMM или другой
...
Production brownfield rerun NC-3812 запрещён до merge этого runtime-locus fix с
зелёным CI. После merge для #41 всё ещё нужен один external-evidence adoption
rerun; clean spare-hardware matrix остаётся в #16.
...
Read-only device discovery реализован fixture-first. `routerkit devices status` показывает live Netcraze/Keenetic adapter как `contract_unverified`, пока hardware probe не подтвердит, нужен ли target firmware local CLI, `/rci`, другой structured interface или их сочетание. Offline validation использует protected synthetic inventory: `routerkit devices discover --inventory-file PATH` или `routerkit devices select --inventory-file PATH`; JSON помечает local-sensitive names, addresses, stable identifiers, source names и raw errors, а `--public-evidence` разрешён только для discover JSON и использует schema-controlled source categories плюс counted generic error codes. Nonzero selection fail-closed, если не все required sources имеют `supported`, есть sanitized errors или выбранный device не имеет valid unicast MAC identity; fixture data не может повысить vendor, router, unknown или IP-only identifiers до assignment trust. `setup --discover-devices --device-inventory-file PATH` добавляет тот же read-only selection stage после strict planning и до apply confirmation. Option `0`, blank input и EOF означают no device assignment. В #21 нет command execution, policy write, proxy write, default-policy change, active scan, persisted inventory, persisted selection handle или device assignment; #15 остаётся write boundary. См. [архитектуру](docs/architecture/device-discovery.ru.md), [исследование интерфейса](docs/research/device-discovery-interface.ru.md) и [read-only hardware probe packet](docs/hardware/device-discovery-probe.ru.md).

Fixture-first программное ядро #15 доступно отдельно: `routerkit netcraze-plan status|plan|simulate`. Generator создаёт durable owner-only `routerkit.local-endpoints.v1` только со slots, фиксированными code-owned labels `primary`/`fallback-1`/`fallback-2`, loopback, ports 1082–1084, enabled и SOCKS5; raw profile names не попадают в manifest. POSIX publication directory обязана иметь exact owner-private `0700`; unsafe directory/file отвергается без chmod/replacement, затем выполняются parent-directory `fsync` и retirement validated stale manifest. Planner проверяет snapshot references и каждый directly supplied selected-device ref, использует общий trusted-unicast MAC helper #21/#15, встраивает полные desired profiles и привязывает plan к exact canonical source snapshot semantics. Simulator не принимает отдельный manifest, до mutation отвергает snapshot mismatch/tampered plan, проверяет actual synthetic objects и доказывает reuse-only idempotent rerun. Local integrity/snapshot fingerprints не попадают в public evidence; отдельный public fingerprint включает minimized digest полного canonical default projection и не является write authorization. Updates существующих объектов и assignment moves остаются blocked в этом fixture-first path. При сочетании `setup --plan-netcraze` с `--apply` confirmation и `--yes` output явно сообщают, что Netcraze actions исключены. В integrated setup всё ещё нет native Netcraze apply stage. Hardware-confirmed standalone execution использует сохранённый local `ndmc` adapter или protected external transaction protocol для official MCP/RMM без arbitrary Entware shell; см. [transport architecture](docs/architecture/netcraze-external-transport.ru.md) и [live runbook](docs/live-install-runbook.ru.md). Fixture-first вердикт остаётся `SOFTWARE_PLAN_CORE_READY_HARDWARE_WRITE_CONTRACT_PENDING`. См. [planning architecture](docs/architecture/netcraze-policy-plan.ru.md), [research](docs/research/netcraze-policy-interface.ru.md) и [hardware packet](docs/hardware/netcraze-policy-contract.ru.md).

Consolidated offline hardware-canary package связывает #21, #15 и полную matrix #16, не включая hardware access. `python3 scripts/routerkit-hardware-canary.py status|validate|render|matrix` читает только committed secret-free packet и repository references; consolidated shell probe умеет только печатать status/phases/readiness. Валидный repository gate сообщает `READY_FOR_HARDWARE_CANARY` вместе с `hardware_validated=false` и `live_contract_confirmed=false`. Operator flow ограничен 120 минутами и резервирует 15 минут на cleanup/device return; он покрывает read contracts, explicitly authorized disposable writes, full alpha.16 software path, rerun, safe failures, reboot/recovery, evidence, rollback и cleanup. Это development/operator tooling, а не часть normal `routerkit setup`. См. [основной runbook](docs/hardware/netcraze-hardware-canary.ru.md), [checklist](docs/hardware/netcraze-canary-checklist.ru.md) и [readiness architecture](docs/architecture/hardware-canary-readiness.ru.md).

Standalone optional assignment использует защищённый inventory #21 и требует одновременно `--device-inventory-file PATH` и явный `--device-choice N`; `0` означает no assignment. Selection handle не сериализуется и не сохраняется.
...
docs/hardware/platform-compatibility.md     Аудит моделей, архитектур, накопителей и прошивок
docs/hardware/platform-compatibility.ru.md  Русская версия аудита платформ и прошивок
...
- [Совместимость платформ и прошивок](docs/hardware/platform-compatibility.ru.md)
- [Platform and firmware compatibility — EN](docs/hardware/platform-compatibility.md)
