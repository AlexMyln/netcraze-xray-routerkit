...
Fixture-first программное ядро #15 доступно отдельно: `routerkit netcraze-plan status|plan|simulate`. `plan` читает owner-only manifest локальных endpoints и защищённый синтетический router snapshot; `simulate` дополнительно требует `--fixture-simulation`. Generator создаёт атомарный private `routerkit.local-endpoints.v1` только со slots, safe labels, loopback, ports 1082–1084, enabled и SOCKS5 — без upstream и credentials. `setup --plan-netcraze --netcraze-state-file PATH` запускает offline план после optional #21 selection и до apply confirmation. Fixture не может дать ownership/write authority, default policy неизменяема, live adapter и Netcraze apply mode отсутствуют. Вердикт: `SOFTWARE_PLAN_CORE_READY_HARDWARE_WRITE_CONTRACT_PENDING`. См. [архитектуру](docs/architecture/netcraze-policy-plan.ru.md), [research](docs/research/netcraze-policy-interface.ru.md) и [hardware packet](docs/hardware/netcraze-policy-contract.ru.md).
